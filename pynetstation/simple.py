...
class _DataFormat :
    ...
    def _make_event_header( self, size_of_the_rest, timestamp, duration, keycode ) :
        ...
        result_str = struct.pack( b"=sH2L4s", b'D', total_length, timestamp, duration, keycode )     
...
class Netstation :     
    ...
    def SendLocalTime( self, ms_time = None ):
        ...
        message = self._fmt.pack( cmd, ms_time )  
    ...
    def send_event(self, key, timestamp=None, label=None, description=None, table=None, pad=False):
        ...
        print('composing event message')
        cmd = bytes(key, "utf-8")
        print(cmd)
        message = self._data_fmt.pack(cmd, timestamp, label, description, table, pad)
